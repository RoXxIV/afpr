// ---------------------------------------------------------------------------
// Commandes Mosquitto utiles (remplace X par l'IP du broker) :
//
// Écouter tous les bancs :
//   mosquitto_sub -h 192.168.1.X -t "#" -v
//
// Écouter un banc spécifique :
//   mosquitto_sub -h 192.168.1.X -t "banc1/bms/data" -v
//
// Format CSV reçu sur banc1/bms/data :
//   voltage,current,soc,temperature,maxCellN,maxCellV,minCellN,minCellV,
//   dischargedCapacity,dischargedEnergy,cell1,...,cell15
// ---------------------------------------------------------------------------
...
void initBancs()
{
  for (int i = 0; i < NB_BANCS; i++)
  {
    ...
    for (int j = 0; j < NB_CELLULES; j++)
      bancs[i].cellVoltages[j] = random(3100, 3400);
  }
}
...
void publishBanc(int i)
{
  ...
  for (int j = 0; j < NB_CELLULES; j++)
  {
    if (bancs[i].cellVoltages[j] > maxCellV)
    {
      maxCellV = bancs[i].cellVoltages[j];
      maxCellN = j + 1;
    }
    if (bancs[i].cellVoltages[j] < minCellV)
    {
      minCellV = bancs[i].cellVoltages[j];
      minCellN = j + 1;
    }
  }
